...
enum {
	...
	A_SET_NO_DEALLOC,	/* set no-dealloc flag of the loop device */
...
static struct colinfo infos[] = {
	...
	[COL_NO_DEALLOC]  = { "NO-DEALLOC",   1, SCOLS_FL_RIGHT, N_("no-dealloc flag set"), SCOLS_JSON_BOOLEAN},
...
static int set_scols_data(struct loopdev_cxt *lc, struct libscols_line *ln)
{
	...
	for (i = 0; i < ncolumns; i++) {
		...
		case COL_DIO:
			...
			break;
		case COL_NO_DEALLOC:
			p = loopcxt_is_no_dealloc(lc) ? "1" : "0";
			break;
		case COL_PARTSCAN:
...
static void __attribute__((__noreturn__)) usage(void)
{
	...
	fputs(_("     --no-dealloc[=<on|off>]   don't deallocate disk space of discarded blocks\n"), out);
...
int main(int argc, char **argv)
{
	...
	unsigned long use_no_dealloc = 0, set_no_dealloc = 0;
	...
	static const struct option longopts[] = {
		...
		{ "no-dealloc",   optional_argument, NULL, OPT_NO_DEALLOC },
	...
	while ((c = getopt_long(argc, argv, "ab:c:d:Dfhj:JlLno:O:PrvV",
		...
		case OPT_DIO:
			...
			break;
		case OPT_NO_DEALLOC:
			use_no_dealloc = set_no_dealloc = 1;
			if (optarg)
				use_no_dealloc = parse_switch(optarg, _("argument error"), "on", "off", NULL);
			if (use_no_dealloc)
				lo_flags |= LO_FLAGS_NO_DEALLOC;
			break;
		case 'v':
			break;
	...
	if (list && !ncolumns) {
		...
		columns[ncolumns++] = COL_NO_DEALLOC;
	...
	if (!act && optind + 1 == argc) {
		 ...
		 * losetup [--list] <device>
		 * OR
		 * losetup {--direct-io[=off]|--no-dealloc[=off]|--logical-blocksize=size}... <device>
		 */
		if (set_dio) {
			act = A_SET_DIRECT_IO;
			lo_flags &= ~LO_FLAGS_DIRECT_IO;
		} else if (set_no_dealloc) {
			act = A_SET_NO_DEALLOC;
			lo_flags &= ~LO_FLAGS_NO_DEALLOC;
	...
	case A_SET_DIRECT_IO:
		...
		if (res)
			warn(_("%s: set direct io failed"),
			        loopcxt_get_device(&lc));
		break;
	case A_SET_NO_DEALLOC:
		res = loopcxt_get_info(&lc) ? 0 : -EINVAL;
		if (res == 0) {
			if (use_no_dealloc)
				lc.config.info.lo_flags |= LO_FLAGS_NO_DEALLOC;
			else
				lc.config.info.lo_flags &= ~LO_FLAGS_NO_DEALLOC;
			res = loopcxt_ioctl_status(&lc);
		}
		if (res)
			warn(_("%s: set no-dealloc failed"),
			        loopcxt_get_device(&lc));
		break;
